...
void gameCore::startGameLoop() {
	...
	while (HI2::aptMainLoop() && !states.empty() && !_exit) {
		...
		double seconds = (double)(std::chrono::duration_cast<std::chrono::microseconds>(currentTick - lastTick).count())/1000000.0f;
		double secOg = seconds;
		if(seconds>(double)(1.0f/29.0f))
		{
			seconds=(1.0f/29.0f);
		}
		
		states.top()->input(seconds);

		states.top()->update(seconds);

		states.top()->draw(secOg);
